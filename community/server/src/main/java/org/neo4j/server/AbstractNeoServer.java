...
import com.globigdata.neo4j.util.CypherServerMoudle;
import com.globigdata.neo4j.util.SocketTcpServer;
...
public abstract class AbstractNeoServer implements NeoServer
{
            ...
            GraphDatabaseFacadeFactory.Dependencies dependencies, LogProvider logProvider )
    {
        ...
        ////////////////////TCP SERVER//////////////////////////
        if(CommunityEntryPoint.isRunTcpServer) {
    ...
    public void init()
    {
        ...
        if(CommunityEntryPoint.isRunTcpServer) {
            ...
            this.tcpServer.setAddress(tcpAddress);
            this.tcpServer.setPort(tcpPort);
        }
    ...
    public void start() throws ServerStartupException
    {
        ...
        try
        {
            ...
            ////////////////////TCP SERVER//////////////////////////
            if(CommunityEntryPoint.isRunTcpServer) {
                TcpServer.Dependencies dependencies = new TcpServer.Dependencies();
                dependencies.add(CypherExecutor.class, cypherExecutor);
                dependencies.add(InputFormat.class, new JsonFormat());
               ...
               // org.neo4j.server.rest.repr.formats.JsonFormat, output format class = org.neo4j.server.rest.repr.OutputFormat
                ...
                CypherServerMoudle cypherServerMoudle = new CypherServerMoudle();
                cypherServerMoudle.setDependencies(dependencies);
                cypherServerMoudle.start();
                tcpServer.setDependencies(dependencies);

                tcpServer.addMoudle(TcpMoudleType.CYPHER, cypherServerMoudle);
                tcpServer.start();

//                ExecutionEngine executionEngine = this.cypherExecutor.getExecutionEngine();
//                Result result = executionEngine.executeQuery("MATCH (n) RETURN n", , cypherExecutor.createTcpSession());
